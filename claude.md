...
6. **Units:** every temperature field (setpoints idx 4/18, current temps idx 14/26, limits,
   hysteresis, compensation) is sent in the displayed unit (`unit` = idx 9, 0=°C 1=°F).
   Switching unit means sending `02` with **all** temp fields converted (`Fridge.setUnit`).
...
matched the official app). Layout re-derived from the decompiled app (see `PROTOCOL.md` §4): current
temps are `idx14` (fridge) and `idx26` (freezer), whole degrees, in the displayed unit; `idx15` is
battery %, `idx16/17` is supply voltage.
